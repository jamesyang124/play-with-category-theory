...
It defines that, if there has a morphism `f: A -> B` and there has another morphism `g` which takes `f`'s output object `B` as input, which denote as `g: B -> C`, then they must be **closed** by a composition `f . g`.
